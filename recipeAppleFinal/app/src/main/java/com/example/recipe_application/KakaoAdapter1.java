...
class KakaoAdapter1 extends BaseAdapter {
    Context context;     // 현재 화면의 제어권자
    int layout;              // 한행을 그려줄 layout
    ArrayList<Bitmap> al;     // 다량의 데이터
    LayoutInflater inf; // 화면을 그려줄 때 필요
    public KakaoAdapter1(Context context, int layout, ArrayList<Bitmap> al) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        iv.setImageBitmap(al.get(position));
        return convertView;
    }
}
